...
class AssetWeights(BaseModel):
    ...
    benchmark_weights: Optional[Dict[str, float]] = Field(None, description="Benchmark weights")
...
class ConcentrationRepairRequest(BaseModel):
    ...
    max_concentration: float = Field(default=0.05, description="Maximum single asset concentration")
    sector_caps: Optional[Dict[str, float]] = Field(None, description="Sector concentration limits")
    country_caps: Optional[Dict[str, float]] = Field(None, description="Country concentration limits")
...
def load_policies() -> Dict[str, Any]:
    ...
    if POLICIES is None:
        try:
            with open("docs/policies.yaml", "r") as f:
                POLICIES = yaml.safe_load(f)
            
            # Load mapping files
            SECTOR_MAP = pd.read_csv("docs/sector_map.csv").set_index("symbol")["sector"].to_dict()
            COUNTRY_MAP = pd.read_csv("docs/country_map.csv").set_index("symbol")["country"].to_dict()
...
def estimate_mu_sigma(returns_data: pd.DataFrame, lookback: int = 252) -> tuple:
    ...
    weights = np.array([(1 - alpha) * alpha**i for i in range(len(recent_returns))][::-1])
    weights = weights / weights.sum()
    
    mu = (recent_returns * weights.reshape(-1, 1)).sum(axis=0)
    
    # Covariance matrix using EWMA
    sigma = recent_returns.ewm(alpha=1-alpha).cov().iloc[-len(recent_returns.columns):, :]
...
def concentration_repair(weights: Dict[str, float], max_concentration: float = 0.05,
                        sector_caps: Optional[Dict[str, float]] = None,
                        country_caps: Optional[Dict[str, float]] = None) -> Dict[str, float]:
    """
    ...
    if country_caps and COUNTRY_MAP:
        for country, cap in country_caps.items():
            country_assets = [i for i, asset in enumerate(assets) 
                            if COUNTRY_MAP.get(asset) == country]
            if country_assets:
    ...
    try:
        ...
        if problem.status == cp.OPTIMAL:
            repaired_weights = {asset: float(w.value[i]) for i, asset in enumerate(assets)}
            logger.info(f"Concentration repair completed. HHI: {compute_hhi(repaired_weights):.4f}")
...
def mean_variance_optimize(mu: np.ndarray, sigma: np.ndarray, risk_aversion: float = 3.0,
...
async def optimize_portfolio(request: OptimizationRequest):
    ...
    try:
        ...
        # Format response
        portfolio = {asset: float(weight) for asset, weight in 
                    zip(request.assets, optimal_weights)}
        
        # Calculate metrics
        portfolio_return = float(np.dot(optimal_weights, mu))
        portfolio_risk = float(np.sqrt(np.dot(optimal_weights, np.dot(sigma, optimal_weights))))
...
async def repair_portfolio_concentration(request: ConcentrationRepairRequest):
    ...
    try:
        ...
        logger.error(f"Concentration repair failed: {e}")
        raise HTTPException(500, f"Concentration repair failed: {str(e)}")
...
async def calculate_risk_metrics(weights: AssetWeights):
    ...
    try:
        ...
        if weights.benchmark_weights:
            active_weights = {asset: portfolio_weights.get(asset, 0) - 
                            weights.benchmark_weights.get(asset, 0)
                            for asset in set(list(portfolio_weights.keys()) + 
        ...
        raise HTTPException(500, f"Risk calculation failed: {str(e)}")
