...
def generate_sample_data(num_assets: int = 10) -> dict[str, Any]:
    ...
    return {
        ...
        "expected_returns": {
            asset: float(ret)
            for asset, ret in zip(assets, expected_returns, strict=False)
        },
        "covariance_matrix": covariance_matrix.tolist(),
        "volatilities": {
            asset: float(vol) for asset, vol in zip(assets, volatilities, strict=False)
        },
...
def create_portfolio_chart(
    weights: dict[str, float], title: str = "Portfolio Allocation"
) -> go.Figure:
...
def create_efficient_frontier_chart(
    returns: list[float], risks: list[float], optimal_point: tuple = None
) -> go.Figure:
...
def main():
    ...
    # Header
    st.markdown(
        '<h1 class="main-header">📊 Financial Decision Engine</h1>',
        unsafe_allow_html=True,
    )
...
def portfolio_optimization_page():
    ...
    with col1:
        ...
        returns_df = pd.DataFrame.from_dict(
            data["expected_returns"], orient="index", columns=["Expected Return"]
        )
        returns_df["Expected Return"] = returns_df["Expected Return"].apply(
            lambda x: f"{x:.2%}"
        )
    ...
    with col2:
        ...
        vol_df = pd.DataFrame.from_dict(
            data["volatilities"], orient="index", columns=["Volatility"]
        )
            ...
            if "error" not in result:
                ...
                with col1:
                    ...
                    portfolio_df = pd.DataFrame.from_dict(
                        portfolio_weights, orient="index", columns=["Weight"]
                    )
                    portfolio_df["Weight"] = portfolio_df["Weight"].apply(
                        lambda x: f"{x:.2%}"
                    )
                    ...
                    fig = create_portfolio_chart(
                        result["portfolio"], "Optimized Portfolio Allocation"
                    )
                ...
                with col2:
                    ...
                    st.metric(
                        "Concentration (HHI)", f"{metrics['concentration_hhi']:.4f}"
                    )
                ...
                st.error(
                    "❌ Optimization failed. Please check your inputs and try again."
                )
...
def risk_analysis_page():
    ...
    with col1:
        ...
        if st.button("🛠️ Repair Concentration"):
            ...
            repair_result = call_api_endpoint(
                "/repair-concentration", "POST", repair_request
            )
            ...
            try:
                ...
                if result["context_documents"]:
                    ...
                    for i, doc in enumerate(result["context_documents"]):
                        with st.expander(
                            f"Source {i + 1} (Relevance: {doc['relevance_score']:.3f})"
                        ):
