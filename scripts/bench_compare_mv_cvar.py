...
def mean_variance_optimize(mu: np.ndarray, sigma: np.ndarray, risk_aversion: float = 3.0) -> np.ndarray:
...
def cvar_optimize(mu: np.ndarray, scenarios: np.ndarray, alpha: float = 0.05, 
                 return_target: float = None) -> np.ndarray:
...
def generate_scenarios(mu: np.ndarray, sigma: np.ndarray, n_scenarios: int = 1000) -> np.ndarray:
...
def calculate_portfolio_metrics(weights: np.ndarray, mu: np.ndarray, sigma: np.ndarray,
                               scenarios: np.ndarray = None) -> Dict[str, float]:
...
def run_benchmark_comparison(n_assets: int = 10, n_trials: int = 50) -> pd.DataFrame:
    ...
    for trial in range(n_trials):
        ...
        for risk_aversion in risk_aversions:
            ...
            cvar_metrics = calculate_portfolio_metrics(cvar_weights, mu, sigma, scenarios)
...
def analyze_results(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    mv_avg = {
        ...
        for col in mv_results.columns if col.startswith('mv_')
    }
    
    cvar_avg = {
        col.replace('cvar_', ''): cvar_results[col].mean() 
        for col in cvar_results.columns if col.startswith('cvar_')
    }
    
    analysis['average_metrics'] = {
        'mean_variance': mv_avg,
        'cvar': cvar_avg
    }
...
def create_benchmark_plots(df: pd.DataFrame, output_dir: Path):
    ...
    axes[0, 0].scatter(mv_data['mv_volatility'], mv_data['mv_expected_return'], 
                      alpha=0.6, label='Mean-Variance', c='blue')
    axes[0, 0].scatter(cvar_data['cvar_volatility'], cvar_data['cvar_expected_return'], 
                      alpha=0.6, label='CVaR', c='red')
    ...
    axes[1, 1].bar(['Mean-Variance', 'CVaR'], 
...
def main():
    ...
    print(f"\nSolve Times:")
    print(f"  Mean-Variance: {solve_times['mean_variance_avg']:.4f}s ± {solve_times['mean_variance_std']:.4f}s")
    print(f"  CVaR:          {solve_times['cvar_avg']:.4f}s ± {solve_times['cvar_std']:.4f}s")
    ...
    print("• Risk-return tradeoffs depend on market conditions and investor preferences")
