...
def apply_scenario_to_portfolio(
    ...
    for asset, _weight in portfolio_weights.items():
...
def run_scenario_analysis(
    ...
    for scenario_id, scenario in scenarios.items():
        ...
        result = {
            ...
            **{
                f"return_{asset}": asset_returns.get(asset, 0)
                for asset in portfolio_weights.keys()
            },
        }
...
def calculate_risk_metrics(scenario_results: pd.DataFrame) -> dict[str, float]:
    ...
    expected_return = (
        scenario_results["portfolio_return"] * scenario_results["probability"]
    ).sum()

    # Volatility (probability-weighted)
    variance = (
        (scenario_results["portfolio_return"] - expected_return) ** 2
        * scenario_results["probability"]
    ).sum()
    volatility = np.sqrt(variance)

    # Value at Risk (95% confidence)
    sorted_returns = scenario_results["portfolio_return"].sort_values()
    cumulative_prob = scenario_results.set_index(sorted_returns.index)[
        "probability"
    ].cumsum()
    ...
    if len(cvar_scenarios) > 0:
        cvar_95 = (
            cvar_scenarios["portfolio_return"] * cvar_scenarios["probability"]
        ).sum() / cvar_scenarios["probability"].sum()
    else:
        cvar_95 = var_95
    ...
    if len(upside_scenarios) > 0:
        upside_return = (
            upside_scenarios["portfolio_return"] * upside_scenarios["probability"]
        ).sum() / upside_probability
    else:
        upside_return = 0

    if len(downside_scenarios) > 0:
        downside_return = (
            downside_scenarios["portfolio_return"] * downside_scenarios["probability"]
        ).sum() / downside_probability
    else:
        downside_return = 0
    ...
    return {
        ...
        "gain_loss_ratio": abs(upside_return / downside_return)
...
def generate_monte_carlo_scenarios(
    n_scenarios: int = 10000, correlation_matrix: np.ndarray = None
) -> pd.DataFrame:
    ...
    if correlation_matrix is None:
        ...
        correlation_matrix = correlation_matrix + np.random.normal(
            0, 0.1, (n_factors, n_factors)
        )
    ...
    shocks = np.random.multivariate_normal(
        mean=np.zeros(6), cov=correlation_matrix, size=n_scenarios
    )

    # Scale shocks to realistic ranges
    shock_scales = np.array(
        [0.25, 0.15, 0.03, 0.8, 0.1, 0.2]
    )  # equity, bond, credit, vix, currency, commodity
...
def main():
    ...
    for _, row in scenario_results.iterrows():
        print(
            f"  {row['scenario_name']:20} {row['portfolio_return_pct']:6.2f}% "
            f"(prob: {row['probability']:.1%})"
        )

    print("\nRisk Metrics:")
    print(
        f"  Expected Return:     {risk_metrics['expected_return']:.4f} ({risk_metrics['expected_return'] * 100:.2f}%)"
    )
    ...
    print(
        f"  Max Drawdown:        {risk_metrics['max_drawdown']:.4f} ({risk_metrics['max_drawdown'] * 100:.2f}%)"
    )
    ...
    for _, scenario in mc_scenarios.iterrows():
        asset_returns = apply_scenario_to_portfolio(
            portfolio, characteristics, scenario.to_dict()
        )
        portfolio_return = sum(
    ...
    print(
        f"  Mean Return:         {np.mean(mc_results):.4f} ({np.mean(mc_results) * 100:.2f}%)"
    )
    print(f"  Volatility:          {np.std(mc_results):.4f}")
    print(
        f"  VaR (95%):          {np.percentile(mc_results, 5):.4f} ({np.percentile(mc_results, 5) * 100:.2f}%)"
    )
    print(
        f"  CVaR (95%):         {np.mean(mc_results[mc_results <= np.percentile(mc_results, 5)]):.4f}"
    )
    print(
        f"  Worst Case:          {np.min(mc_results):.4f} ({np.min(mc_results) * 100:.2f}%)"
    )
    print(
        f"  Best Case:           {np.max(mc_results):.4f} ({np.max(mc_results) * 100:.2f}%)"
    )
    ...
    mc_df = pd.DataFrame(
        {"scenario_id": range(len(mc_results)), "portfolio_return": mc_results}
    )
