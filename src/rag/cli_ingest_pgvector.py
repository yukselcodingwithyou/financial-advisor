...
def ingest_documents(rag: PGVectorRAG, documents: list[dict[str, Any]]) -> None:
    ...
    for i, doc in enumerate(documents):
        try:
            ...
            rag.add_document(
                content=doc["content"], metadata=doc["metadata"], doc_id=doc_id
            )
            logger.info(
                f"Ingested document {doc_id}: {doc['metadata'].get('title', 'Untitled')}"
            )
...
def main():
    ...
    parser.add_argument(
        "--sample", action="store_true", help="Ingest sample financial documents"
    )

    parser.add_argument(
        "--file", type=str, help="Path to JSON file containing documents to ingest"
    )
    ...
    parser.add_argument(
        "--test-query", type=str, help="Test query to run after ingestion"
    )
