...
class PGVectorRAG:
    ...
    def __init__(self, connection_string: str | None = None):
        ...
        self.connection_string = connection_string or os.getenv(
            "PGVECTOR_URL", "postgresql://postgres:password@localhost:5432/vector_db"
        )
    ...
    def delete_document(self, doc_id: str) -> bool:
        ...
        try:
            with psycopg2.connect(self.connection_string) as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        f"DELETE FROM {self.table_name} WHERE doc_id = %s;", (doc_id,)
                    )
    ...
    def query(
        self, query: str, top_k: int = 3, metadata_filter: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        """
...
def test_pgvector_rag() -> None:
    ...
    try:
        ...
        print(
            f"PGVectorRAG test failed (this is expected if PostgreSQL/pgvector not available): {e}"
        )
