...
def efficient_frontier(
    mu: np.ndarray, sigma: np.ndarray, num_points: int = 50
    ...
    float(np.dot(w_min_var, np.dot(sigma, w_min_var)))
    ...
    for target_ret in target_returns:
        try:
            ...
            constraints = [cp.sum(w) == 1, w >= 0, mu.T @ w == target_ret]
                ...
                try:
                    ...
                    if problem.status == cp.OPTIMAL:
                        portfolio_risk = float(
                            np.sqrt(np.dot(w.value, np.dot(sigma, w.value)))
                        )
...
class TestMeanVarianceOptimization:
    ...
    def sample_data(self):
        ...
        return {"mu": mu, "sigma": sigma, "n_assets": n_assets}
    ...
    def test_mean_variance_with_constraints(self, sample_data):
        ...
        weights = mean_variance_optimize(
            mu, sigma, risk_aversion=3.0, constraints=constraints
        )
    ...
    def test_optimization_numerical_stability(self):
        ...
        sigma = np.array(
            [[0.01, 0.009, 0.008], [0.009, 0.01, 0.009], [0.008, 0.009, 0.01]]
        )
